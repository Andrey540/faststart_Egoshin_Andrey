...
enum class NumberOfChocolateSlice
{
	One,
	Two,
	Three,
...
{
public:
	CChocolateSlice(IBeveragePtr && beverage, NumberOfChocolateSlice numberOfChocolateSlice)
		: CCondimentDecorator(move(beverage))
		, m_numberOfChocolateSlice(numberOfChocolateSlice)
	{}
protected:
	double GetCondimentCost()const override
	{
		return 10 * (static_cast<int>(m_numberOfChocolateSlice) + 1);
	}
	std::string GetCondimentDescription()const override
	{
		return "Chocolate Slice x " + std::to_string(static_cast<int>(m_numberOfChocolateSlice) + 1);
	}
private:
	NumberOfChocolateSlice m_numberOfChocolateSlice;
};
