...
struct MakeLemon
{
	...
	unique_ptr<CLemon> operator()(IBeveragePtr && beverage)const
	{
...
int main()
{
	...
	{
		auto beverage = 
			make_unique<CMilkshake>(MilkshakePortionType::Big)	// Наливаем молочный коктейль
			<< MakeCondiment<CSyrup>(SyrupType::Maple)	// заливаем кленовым сиропом
			<< MakeCondiment<CLiquor>(LiquorType::Walnut)	// заливаем ореховый ликёр
	...
	{
		auto beverage =
			make_unique<CLatte>(CoffePortionType::Double)	// Наливаем двойное лате
			<< MakeCondiment<CCream>() 		// пзаливам сливки
			<< MakeCondiment<CChocolateSlice>(NumberOfChocolateSlice::Three);
